...
class UnitTest(unittest.TestCase):
    ...
    def test_actual_len_word(self):
        input = 's_ n_ esh'
        output = 's_n_esh'
        self.assertEqual(output, actual_len_word(input))

    def test_winner(self):
        input = {'sandesh': 25, 'sandy': 25}
        output = 'sandesh, and sandy are the winners.\n'
        self.assertEqual(output, get_print(winner,input))

    def test_get_available_letters(self):
        input = ["a",'e','i','o','u']
        output = 'bcdfghjklmnpqrstvwxyz'
        self.assertEqual(output, get_available_letters(input))
